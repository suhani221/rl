...
class MyopicPolicy(RMABPolicy):
    """Class for implementing a Myopic RMAB policy."""
    
    policy_key_str = 'Myopic Policy'

    @staticmethod
    def policy_key() -> str:
        """Return the name of the policy."""
        return MyopicPolicy.policy_key_str

    def get_actions(self, current_states: np.ndarray = None, remaining_horizon: int = None) -> np.ndarray:
        """Return actions to take in the RMAB based on the highest immediate rewards.
        
        Args:
          current_states: current state of the arms in the RMAB
          remaining_horizon: time periods remaining in the simulation (unused in Myopic Policy)

        Returns:
          actions: n_arms-length integer array of actions to take in the RMAB
        """
        if current_states is None:
            raise ValueError('current_states must be provided to MyopicPolicy.get_actions()')

        # Initialize actions array with zeros
        actions = np.zeros(self.n_arms, dtype=np.int32)

        # Calculate the immediate reward for each arm
        immediate_rewards = self.rewards[np.arange(self.n_arms), current_states]

        # Find the indices of the arms with the highest rewards up to the budget limit
        top_arms = np.argsort(immediate_rewards)[-self.budget:]

        # Set these top arms to action 1 (assuming binary actions: 0 or 1)
        actions[top_arms] = 1

        return actions

class RoundRobinPolicy(RMABPolicy):
    """Class for implementing a Round Robin RMAB policy."""
    
    policy_key_str = 'Round Robin Policy'

    def __init__(self, n_arms: int, **kwargs) -> None:
        """Initialize the Round Robin policy class.
        
        Args:
          n_arms: number of arms in the RMAB
        """
        super().__init__(n_arms)
        self.current_arm = 0  # Start with the first arm

    @staticmethod
    def policy_key() -> str:
        """Return the name of the policy."""
        return RoundRobinPolicy.policy_key_str

    def get_actions(self, current_states: np.ndarray = None, remaining_horizon: int = None) -> np.ndarray:
        """Return actions to take in the RMAB using Round Robin.
        
        Args:
          current_states: current state of the arms in the RMAB (unused in Round Robin)
          remaining_horizon: time periods remaining in the simulation (unused in Round Robin)

        Returns:
          actions: n_arms-length integer array of actions to take in the RMAB
        """
        actions = np.zeros(self.n_arms, dtype=int)
        actions[self.current_arm] = 1  # Activate the current arm
        self.current_arm = (self.current_arm + 1) % self.n_arms  # Move to the next arm
        return actions
